  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 via-blue-50/30 to-white">
      ...
      <main className="max-w-7xl mx-auto px-6 pt-24 pb-20 space-y-16">
        ...
        {/* ─── EPOCH INPUT ─── */}
        <div className="p-5 bg-white/60 backdrop-blur border border-white/40 rounded-2xl shadow-sm">
          <div className="flex items-end gap-3 mb-2">
            <div>
              <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Octant Epoch Number</label>
            ...
            <div className="flex gap-1 pb-1">
              {[3, 4, 5, 6].map((e) => (
                <button key={e} onClick={() => setSelectedEpoch(e)}
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium transition ${selectedEpoch === e ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-600 hover:bg-blue-50"}`}>
                  Epoch {e}
                </button>
              ))}
            </div>
          </div>
          <p className="text-xs text-slate-400">Octant runs 90-day funding epochs. Each epoch has projects, allocations, and rewards. Select an epoch to analyze below. Epoch 5 has the most data (30 projects, 1,902 donations).</p>
        </div>
        ...
        <section id="evaluate" className="scroll-mt-24">
          ...
          <Card>
            <div className="mb-4 p-3 rounded-xl bg-blue-50/50 border border-blue-100 text-xs text-slate-600">
              ...
              <p>Enter any public goods project name and description. The AI will evaluate it across 8 dimensions (Impact, Team, Innovation, Sustainability, Ecosystem Alignment, Transparency, Community, Risk) and provide a scored assessment with strengths, concerns, and recommendation.</p>
            </div>
            <div className="space-y-3 mb-4">
              ...
              <div>
                ...
                <textarea placeholder="e.g. Octant is a public goods funding platform by Golem Foundation. It stakes 100,000 ETH as validators and distributes staking rewards to community-selected projects via quadratic funding every 90 days." value={evalDesc} onChange={(e) => setEvalDesc(e.target.value)} rows={3}
                  className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" />
              </div>
            </div>
        ...
        <section id="project" className="scroll-mt-24">
          ...
          <Card>
            <div className="mb-4 p-3 rounded-xl bg-blue-50/50 border border-blue-100 text-xs text-slate-600">
              <p className="font-semibold text-slate-700 mb-1">How to use:</p>
              <p>Enter an Octant project address (from the Epoch Analysis table above). This command fetches the project&apos;s full history across all epochs, builds a trust profile, simulates 4 funding mechanisms, and generates an AI deep evaluation with PDF report.</p>
              <p className="mt-1 font-semibold text-slate-700">Where to get addresses:</p>
              <p>Run &quot;Epoch Analysis&quot; above first, then copy any address from the results table.</p>
            </div>
            <div className="mb-4">
              <label className="block text-xs font-semibold text-slate-500 mb-1">Octant Project Address</label>
              <input placeholder="e.g. 0x02Cb3C150BEdca124d0aE8CcCb72fefbe705c953" value={projectAddr} onChange={(e) => setProjectAddr(e.target.value)}
