...
export default function DashboardPage() {
  ...
  return (
    <div className="min-h-screen relative">
      ...
      <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-xl border-b border-slate-100">
        <div className="max-w-7xl mx-auto px-6 h-14 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs font-medium text-slate-400 border-l border-slate-200 pl-3">Dashboard</span>
          </div>
            ...
            {loading ? (
              ...
              <>
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-xs text-slate-500">Octant</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <span className={`w-2 h-2 rounded-full ${ai?.status === "ok" ? "bg-green-400" : "bg-red-400"}`} />
                  <span className="text-xs text-slate-500">AI</span>
                </div>
                <span className="text-xs font-mono text-slate-400 bg-slate-50 px-2 py-0.5 rounded-md">Epoch {currentEpoch || "---"}</span>
              </>
            )}
          </div>
        </div>
      ...
      <div className="sticky top-14 z-40 bg-white/70 backdrop-blur-lg border-b border-slate-100/80">
        <div className="max-w-7xl mx-auto px-6 h-12 flex items-center gap-4">
          <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider whitespace-nowrap">Epoch</label>
          <input type="number" value={selectedEpoch} onChange={(e) => setSelectedEpoch(Number(e.target.value))}
            className="w-20 rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-blue-400" />
          <div className="flex gap-1">
            {[3, 4, 5, 6].map((e) => (
              <button key={e} onClick={() => setSelectedEpoch(e)}
                className={`px-3 py-1 rounded-md text-xs font-medium transition ${selectedEpoch === e ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-600 hover:bg-blue-50"}`}>
                {e}
              </button>
            ))}
          </div>
          <span className="text-xs text-slate-400 hidden sm:inline">90-day funding cycles. Epoch 5 has the most data.</span>
        </div>
      </div>
      ...
      <main className="max-w-7xl mx-auto px-6 pt-8 pb-20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-5">
          ...
          <div id="project" className="bg-white/90 backdrop-blur-sm rounded-2xl border border-white/60 shadow-sm hover:shadow-md transition-shadow p-5 scroll-mt-32">
            ...
            <div className="mb-3">
              <label className="block text-xs font-semibold text-slate-500 mb-1">Octant Project Address</label>
              <input placeholder="0x02Cb3C150BEdca124d0aE8CcCb72fefbe705c953" value={projectAddr} onChange={(e) => setProjectAddr(e.target.value)}
                className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-mono focus:outline-none focus:ring-2 focus:ring-blue-400" />
              <div className="flex gap-1.5 mt-2 flex-wrap">
                ...
                {[
                  ...
                  <button key={ex.addr} onClick={() => setProjectAddr(ex.addr)}
                    className="text-xs px-2 py-0.5 rounded-md bg-slate-100 text-slate-600 hover:bg-blue-50 hover:text-blue-600 transition">
                    {ex.label}
                  </button>
                ))}
              </div>
            ...
            <button onClick={runAnalyzeProject} disabled={projectLoading}
              className="bg-blue-600 hover:bg-blue-700 text-white text-xs font-medium rounded-lg px-4 py-2 transition disabled:opacity-50 flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
                      {[
                        ...
                        return (
                          <div key={dim.key} className={`p-2 rounded-xl border ${isOverall ? "bg-blue-50 border-blue-200" : "bg-slate-50 border-slate-100"}`}>
                ...
                {projectResult.reportPath && (
                  <div className="flex gap-2">
                    <button onClick={() => { const f = String(projectResult.reportPath).split("/").pop(); setViewPdf(`/api/reports/${f}`); }}
                      className="bg-blue-600 hover:bg-blue-700 text-white text-xs font-medium rounded-lg px-4 py-2 transition">
                      View PDF Report
                    </button>
          ...
          <div id="evaluate" className="bg-white/90 backdrop-blur-sm rounded-2xl border border-white/60 shadow-sm hover:shadow-md transition-shadow p-5 scroll-mt-32">
            ...
            <div className="mb-3 p-2.5 rounded-lg bg-blue-50/50 border border-blue-100 text-xs text-slate-600">
            ...
            <div className="space-y-2.5 mb-3">
              <div>
                ...
                <input placeholder="e.g. Octant, Gitcoin Grants, Protocol Guild" value={evalName} onChange={(e) => setEvalName(e.target.value)}
                  className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-blue-400" />
              </div>
              <div>
            ...
            {evalResult && <pre className="mt-3 p-3 rounded-lg bg-slate-50 border border-slate-100 text-xs whitespace-pre-wrap overflow-x-auto max-h-80">{evalResult}</pre>}
          </div>
        </div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
                      {[
                        ...
                        const isOverall = dim.key === "overallScore";
                        return (
                          <div key={dim.key} className={`p-2.5 rounded-xl border ${isOverall ? "bg-blue-50 border-blue-200 col-span-2 sm:col-span-1" : "bg-slate-50 border-slate-100"}`}>
                            <p className="text-xs text-slate-500">{dim.label}{dim.weight ? ` (${dim.weight})` : ""}</p>
        ...
        <div id="reports" className="bg-white/90 backdrop-blur-sm rounded-2xl border border-white/60 shadow-sm hover:shadow-md transition-shadow p-5 scroll-mt-32">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-slate-400 font-mono">Generated intelligence reports</p>
            </div>
            <button onClick={() => getReports().then(setReports).catch(() => {})}
              className="bg-blue-600 hover:bg-blue-700 text-white text-xs font-medium rounded-lg px-4 py-2 transition flex items-center gap-2">
              <span className="font-mono text-xs opacity-70">$</span>
              <span>Refresh</span>
            </button>
          </div>
          {reports?.reports?.length ? (
            ...
            <p className="text-xs text-slate-400">No reports yet. Run analyze-project to generate PDF reports.</p>
          )}
        </div>
